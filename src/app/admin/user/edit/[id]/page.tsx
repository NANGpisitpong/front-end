    ...
    <div className="edit-wrapper">
      <div className="edit-card" id="editCard">
        <div className="edit-header">
          ...
          <p className="subtitle">แก้ไขสมาชิก #{id} — อัปเดตแบบเรียลไทม์</p>
        </div>
        ...
        {loading ? (
          ...
          <>
            ...
            <form onSubmit={handleUpdateSubmit} className="edit-form" noValidate>
              <div className="form-section">
                <h3>ข้อมูลส่วนตัว 📝</h3>
                <label>
                  คำนำหน้าชื่อ
                  <select value={prefix} onChange={e => { setPrefix(e.target.value); setDirty(d => ({ ...d, prefix: true })) }} required>
                    <option value="">-- เลือก --</option>
                    {!honorifics.includes(prefix as any) && prefix && <option value={prefix}>{prefix}</option>}
                    {honorifics.map(h => <option key={h} value={h}>{h}</option>)}
                  </select>
                </label>
                <label>
                  ชื่อ(จริง)
                  <input type="text" value={firstName} onChange={e => { setFirstName(e.target.value); setDirty(d => ({ ...d, firstName: true })) }} required />
                </label>
                <label>
                  นามสกุล(จริง)
                  <input type="text" value={lastName} onChange={e => { setLastName(e.target.value); setDirty(d => ({ ...d, lastName: true })) }} required />
                </label>
                <label>
                  เพศ 👩🧑
                  <div className="radio-group">
                    {(['male','female','other'] as Sex[]).map(v => (
                      <label key={v}>
                        <input type="radio" value={v} checked={sex===v} onChange={()=>{ setSex(v); setDirty(d => ({...d,sex:true})) }} required={v==='male'} />
                        {v==='male'?'ชาย':v==='female'?'หญิง':'อื่นๆ'}
                      </label>
                    ))}
                  </div>
                </label>
                <label>
                  วันเกิด
                  <input type="date" value={birthday} onChange={e => { setBirthday(e.target.value); setDirty(d => ({ ...d, birthday: true })) }} />
                </label>
                <label>
                  ที่อยู่
                  <textarea rows={3} value={address} onChange={e => { setAddress(e.target.value); setDirty(d => ({ ...d, address: true })) }} />
                </label>
              </div>
              ...
              <div className="form-section">
                ...
                <label>
                  ชื่อผู้ใช้ (แก้ไขได้)
                  <input type="text" value={username} onChange={e => { setUsername(e.target.value); setDirty(d => ({ ...d, username: true })) }} required />
                </label>
                <label>
                  รหัสผ่าน
                  <input type="password" value={password} onChange={e => { setPassword(e.target.value); setDirty(d => ({ ...d, password: true })) }} required />
                </label>
              </div>
